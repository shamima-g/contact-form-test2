...
| `AppHeader` | `web/src/components/layout/AppHeader.tsx` | Authenticated shell header rendered by the `(protected)` layout on every protected screen. Shows the signed-in user (name + active role), a Sign Out control (`useSession().signOut()`; the layout guard owns the `/sign-in` redirect), and hosts the `RoleSwitcher`. Renders nothing when signed out. Its subtree needs a mounted router (via `RoleSwitcher`), so component tests that render it must mock `next/navigation`. The shared home for shell-level controls. |
| `RoleSwitcher` | `web/src/components/qa/RoleSwitcher.tsx` | QA/demo affordance in the shell header: three always-visible role buttons in a labelled `<section>` (accessible name "QA / Demo"). Clicking calls `useSession().setActiveRole(role)` then `router.push(landingRouteForRole(role))` so landing screen + role-gated actions re-evaluate immediately with no re-sign-in (BR3). Styled as a demo tool, not an end-user setting (NFR-2). |
